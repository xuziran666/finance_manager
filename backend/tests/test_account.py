...
log_dao = LogDAO()
account_service = AccountService(account_dao, log_dao)
...
class TestAccountDAO(unittest.TestCase):
    ...
    def test_create_account(self):
        a = account_dao.create(_uid, "测试账户", "bank", 1000.00)
...
class TestAccountService(unittest.TestCase):
    ...
    def test_add_valid(self):
        succ, r = account_service.add(_uid, "测试服务账户", "alipay", 100.00)
        self.assertTrue(succ)
        self.assertEqual(r["name"], "测试服务账户")

    def test_add_empty_name(self):
        succ, msg = account_service.add(_uid, "", "bank", 0)
        self.assertFalse(succ)
        self.assertEqual(msg, "名称不能为空")

    def test_add_negative_balance(self):
        succ, msg = account_service.add(_uid, "测试", "bank", -100)
        self.assertFalse(succ)
