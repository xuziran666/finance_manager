...
class TransactionService:
    ...
    def __init__(self, account_dao: AccountDAO, transaction_dao: TransactionDAO, log_dao: LogDAO):
    ...
    def add(self, account_id, type_, category, amount, note="", date=None, subcategory=""):
        ...
        with connection_scope() as conn:
            ...
            self.log_dao.add(user_id, "ADD_TRANSACTION", f"账户[{account_id}] {type_}:{amount}", conn=conn)
            return True, transaction

    def delete(self, transaction_id):
        with connection_scope() as conn:
            user_id = get_current_user_id()
            txn = self.transaction_dao.get_by_id(transaction_id, user_id=user_id, conn=conn)
            if not txn:
                return False, "交易不存在"
            self.transaction_dao.delete(transaction_id, user_id=user_id, conn=conn)
    ...
    def transfer(self, source_id, target_id, amount, note=""):
        ...
        with connection_scope() as conn:
            ...
            self.transaction_dao.create(user_id, source_id, "expense", "转账", amount, outgoing_note, today, conn=conn)
            self.transaction_dao.create(user_id, target_id, "income", "转账", amount, incoming_note, today, conn=conn)
            self.log_dao.add(user_id, "TRANSFER", f"从{source_id}转{amount}到{target_id}", conn=conn)
            return True, "转账成功"
