...
class CategoryService:
    ...
    def delete(self, type_, main, sub=""):
        with connection_scope() as conn:
            if self.category_dao.delete(get_current_user_id(), type_, main, sub, conn=conn):
                return True, "删除成功"
            return False, "分类不存在"

    def update(self, old_type, old_main, old_sub, new_type, new_main, new_sub):
        with connection_scope() as conn:
            if self.category_dao.update(get_current_user_id(), old_type, old_main, old_sub, new_type, new_main, new_sub, conn=conn):
                return True, "修改成功"
            return False, "原分类不存在"
